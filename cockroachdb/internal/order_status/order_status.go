...
func ProcessTransaction(db *sql.DB, warehouseId int, districtId int, customerId int) {
	...
	lastOrderQuery := fmt.Sprintf("SELECT O_ID, O_TOTAL_AMOUNT, O_DELIVERY_D, O_ENTRY_D, O_CARRIER_ID FROM ORDERS WHERE O_W_ID=%d AND O_D_ID=%d AND O_C_ID=%d ORDER BY O_ID DESC LIMIT 1", warehouseId, districtId, customerId)
	...
	orderLinesQuery := "SELECT OL_I_ID, OL_SUPPLY_W_ID, OL_QUANTITY FROM ORDER_LINE WHERE OL_W_ID=%d AND OL_D_ID=%d AND OL_O_ID=%d"
	...
	err = crdb.ExecuteTx(context.Background(), db, nil, func(tx *sql.Tx) error {
		fmt.Println(lastOrderQuery);
		if err := tx.QueryRow(lastOrderQuery).Scan(&lastOrderId, &orderAmount, &deliveryDate, &entryDate, &carrierId); err != nil {
			return err
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&orderLine.itemId, &orderLine.supplyWHNumber, &orderLine.quantity); err != nil {
				return err
			}
	...
	for _, orderLine := range orderLines {
		...
		output += fmt.Sprintf("Item number: %d\nSupply Warehouse: %d\nQuantity: %d\nTotal price: %f\nDelivery date: %s\n", 
			orderLine.itemId, orderLine.supplyWHNumber, orderLine.quantity, orderAmount, toPrintDeliveryDate)
	}
	if carrierId.Valid {
